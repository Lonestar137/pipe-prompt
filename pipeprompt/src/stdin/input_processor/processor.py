...
def get_new_row(collected_fields: List[Row], row: str, delimiter: Pattern)->Row:
    search_text = delimiter.search(row)
    if not search_text:
...
def map_raw_data_to_row_class(row_data: str, special_fields_in_row: List[str])->Row:
    ...
    for special_field in special_fields_in_row:
        ...
        value = extract_value_from_special_field(special_field, row_data)
...
#@param field: "tag: value #@"
#@return  value 
def extract_value_from_special_field(field: str, row_data: str)-> str:
    value: str = field.split(':')[1][:-2].strip().replace("{row_data}", row_data)
    return value
